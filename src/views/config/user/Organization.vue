  ...
  <div id="organizationWrapper">
    <div class="organizationLeft">
      ...
      <div class="organizationTreeWrapper">
        <el-tree :data="treeData" node-key="id" ref='organizationTree' :current-node-key="currentNodeKey" :default-expanded-keys='defaultExpanded' :props="defaultProps" :expand-on-click-node='onNode' @node-click='clickNode' :draggable='draggable' :allow-drop="allowDrop" :allow-drag="allowDrag" @node-drop="nodeDrop">
          <span class="custom-tree-node" slot-scope="{node}">
            <span>{{node.label }}</span>
            <span class="iconBox">
              <span class="el-icon-edit operation" @click="clickBtnEdit(node)">
              </span>
              <span class="el-icon-remove-outline operation" @mousedown="clickBtnMove(node)">
              </span>
            </span>
          </span>
        </el-tree>
...
export default {
      ...
      Unitrules: {
        ...
        aliasName: [
          { required: true, message: "必填项", trigger: "change" },
          {
            min: 4,
            max: 4,
            message: "4个大写英文字符的单位简称",
            trigger: "change"
          }
        ],
        aliasName2: [
          { required: true, message: "必填项", trigger: "change" },
          { message: "单位简称的拼音首字母", trigger: "change" }
        ],
        preFactoryId: [
          // { required: true, message: "必填项", trigger: "blur" }
        ],
        factoryType: [{ required: true, message: "必填项", trigger: "change" }],
        regionId: [{ required: true, message: "必填项", trigger: "blur" }],
        detailedAddress: [
          { required: true, message: "必填项", trigger: "change" }
        ],
        contactUser1: [
  ...
  methods: {
      ...
      if (node.data.nodeType === 1) {
        getUnit({
          ...
          if (res.httpStatus === 200) {
            ...
            this.lookFactory("edit", "unit", res.result.result);
          }
        });
      } else {
        getPart({
          ...
          if (res.httpStatus === 200) {
            this.lookFactory("edit", "part", res.result.result);
          }
        });
      }
      // this.lookFactory(confi, type, item)
      this.showForm = true;
    },
    clickBtnMove(node) {
      // tree移动
    ...
    //拖拽成功的回调
    nodeDrop(ourNode, parentNode, position, event) {
      // console.log(ourNode)
      ...
      let obj = {
        sourceNodeId: ourNode.data.id, //当前节点id
        targetParentNodeId: "", //放置后父节点id
        children: []
      };
      if (parentNode.parent.data instanceof Array) {
        // console.log('目标节点没有父级')
        obj = {
          sourceNodeId: ourNode.data.id, //当前节点id
          targetParentNodeId: "", //放置后父节点id
          children: parentNode.parent.data.map((item, index) => {
            return {
              id: item.id,
              // name:item.name,
              sort: index + 1
        ...
        obj = {
          sourceNodeId: ourNode.data.id, //当前节点id
          targetParentNodeId: parentNode.parent.data.id, //放置后父节点id
          children: parentNode.parent.data.children.map((item, index) => {
            return {
              id: item.id,
              // name:item.name,
              sort: index + 1
      ...
      //  obj.targetParentNodeId = parentNode.data.id
    },
    //展开部门
    clickUnit(id, num) {
    ...
    handleSelect(item) {
      ...
      //高亮显示某一节点
      this.$refs.organizationTree.setCurrentKey(item.address);
    },
    //获取单位简称
    getAliasName() {
      // console.log(111)
